...
const ms = require("ms")
const DB = require("../../Schemas/AntiAlt"); // Change this to your own DB
...
module.exports = {
    ...
    async execute( member) {
        ...
        let minAge = ms('7 days'); // Set the minimum age to 7 days
        ...
        if (minAge > diff) { // If the difference is less than the minimum age
            ...
            const logEmbed = new MessageEmbed()
                .setColor("RED")
                .setTitle("ALT FOUND!")
                .setDescription(`A user was kicked for being under 7 days old.\n\n**USER**: ${member.user.tag}\n**ID**: ${member.user.id}`)
                .setTimestamp()
        ...
        async function createAndDeleteWebhook(embedName) {
            await logChannel.createWebhook(member.guild.name, { // Creates a webhook in the logging channel specified before
                ...
                webhook.send({ // Sends the embed through the webhook
                    embeds: [embedName]
                }).then(() => webhook.delete()) // Deletes the webhook and catches the error if any
            });
        }
    }
